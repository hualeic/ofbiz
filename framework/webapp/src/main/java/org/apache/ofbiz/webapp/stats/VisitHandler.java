...
public class VisitHandler {
			...
			if (visit == null) {
				synchronized (session) {
					...
					if (visit == null) {
						...
						if (delegator == null) {
							Debug.logError(
									"Could not find delegator with delegatorName ["
											+ delegatorName
							...
							if (visitor != null) {
								...
								try {
									GenericValue checkVisitor =
											EntityQuery.use(delegator).from("Visitor").where("visitorId", visitorId)
													.queryOne();
	...
	public static GenericValue getVisitor(HttpServletRequest request, HttpServletResponse response) {
		...
		if (!EntityUtilProperties.propertyValueEqualsIgnoreCase("serverstats", "stats.persist.visitor", "false",
				delegator)) {
			...
			if (visitor == null) {
				synchronized (session) {
					...
					if (visitor == null) {
						...
						if (delegator == null) {
							Debug.logError("Could not find delegator in request or with delegatorName ["
									+ delegatorName + "] in session, not creating/getting Visitor entity", module);
						} else {
							...
							if (UtilValidate.isEmpty(cookieVisitorId)) {
								...
								try {
									visitor =
											EntityQuery.use(delegator).from("Visitor")
													.where("visitorId", cookieVisitorId).queryOne();
									if (visitor == null) {
