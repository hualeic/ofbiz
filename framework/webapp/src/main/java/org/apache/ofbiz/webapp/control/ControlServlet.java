...
public class ControlServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		if (Debug.infoOn()) {
			...
			String webappName =
					servletContext.getContextPath().length() != 0 ? servletContext.getContextPath().substring(1) : "";
			Debug.logInfo("Loading webapp [" + webappName + "], located at " + servletContext.getRealPath("/"),
					module);
		}
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (Debug.timingOn()) {
			timer = new UtilTimer();
			timer.setLog(true);
			timer.timerString("[" + rname + "(Domain:" + request.getScheme() + "://" + request.getServerName()
					+ ")] Request Begun, encoding=[" + charset + "]", module);
		}
		...
		if (delegator == null) {
			...
			/*
			 * Uncomment this to enable the EntityClassLoader ClassLoader loader =
			 * EntityClassLoader.getInstance(delegator.getDelegatorName(),
			 * Thread.currentThread().getContextClassLoader()); Thread.currentThread().setContextClassLoader(loader);
			 */
		}
		...
		ServletContextHashModel ftlServletContext =
				new ServletContextHashModel(this, FreeMarkerWorker.getDefaultOfbizWrapper());
		...
		try {
			...
			if (throwable instanceof IOException) {
				...
				// need to serve the error page; a message is logged to record
				// the event
				if (Debug.warningOn())
		...
		// Forward to the JSP
		// if (Debug.infoOn()) Debug.logInfo("[" + rname +
		// "] Event done, rendering page: " + nextPage, module);
		// if (Debug.timingOn()) timer.timerString("[" + rname +
		// "] Event done, rendering page: " + nextPage, module);
		...
		if (errorPage != null) {
			...
			// use this request parameter to avoid infinite looping on errors in
			// the error page...
			if (request.getAttribute("_ERROR_OCCURRED_") == null && rd != null) {
				...
				// NOTE DEJ20070727 after having trouble with all of these, try
				// to get the page out and as a last resort just send something
				// back
				try {
					rd.include(request, response);
				} catch (Throwable t) {
					Debug.logWarning(
							"Error while trying to send error page using rd.include (will try response.getOutputStream "
									+ "or response.getWriter): " + t.toString(), module);

					String errorMessage =
							"ERROR rendering error page [" + errorPage + "], but here is the error text: "
					...
					try {
						...
						try {
							...
							Debug.logWarning(
									"Error while trying to write error message using response.getOutputStream or "
											+ "response.getWriter: " + t.toString() + "; sending error code ["
											+ errorToSend + "], and message [" + errorMessage + "]", module);
							response.sendError(errorToSend, errorMessage);
						} catch (Throwable t3) {
							// wow, still bad... just throw an
							// IllegalStateException with the message and let
							// the servlet container handle it
				...
				String errorMessage =
						"<html><body>ERROR in error page, (infinite loop or error page not found with name ["
								+ errorPage + "]), but here is the text just in case it helps you: "
		...
		// run these two again before the ServerHitBin.countRequest call because
		// on a logout this will end up creating a new visit
		if (response.isCommitted() && request.getSession(false) == null) {
			// response committed and no session, and we can't get a new
			// session, what to do!
			// without a session we can't log the hit, etc; so just do nothing;
			// this should NOT happen much!
			Debug.logError(
					"Error in ControlServlet output where response isCommitted and there is no session (probably "
							+ "because of a logout); not saving ServerHit/Bin information because there is no session "
							+ "and as the response isCommitted we can't get a new one. The output was successful, "
							+ "but we just can't save ServerHit/Bin info.", module);
		} else {
			try {
				UtilHttp.setInitialRequestInfo(request);
				VisitHandler.getVisitor(request, response);
				...
				Debug.logError(t,
						"Error in ControlServlet saving ServerHit/Bin information; the output was successful, but "
								+ "can't save this tracking information. The error was: " + t.toString(), module);
			}
		}
		if (Debug.timingOn())
			timer.timerString("[" + rname + "(Domain:" + request.getScheme() + "://" + request.getServerName()
