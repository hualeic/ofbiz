...
public class GenericEntity implements Map<String, Object>, LocalizedMap<Object>, Serializable,
		Comparable<GenericEntity>, Cloneable {
	...
	public static GenericEntity createGenericEntity(Delegator delegator, ModelEntity modelEntity,
	...
	protected void assertIsMutable() {
		if (!this.mutable) {
			String msg =
					"This object has been flagged as immutable (unchangeable), probably because it came from an Entity Engine cache. Cannot modify an immutable entity object. Use the clone method to create a mutable copy of this object.";
	...
	protected void init(ModelEntity modelEntity) {
		...
		if (this.entityName == null) {
			throw new IllegalArgumentException(
					"Cannot create a GenericEntity with a null entityName in the modelEntity parameter");
		}
	}
	...
	public void refreshFromValue(GenericEntity newValue) throws GenericEntityException {
		...
		if (!thisPK.equals(newPK)) {
			throw new GenericEntityException(
					"Could not refresh value, new value did not have the same primary key; this PK=" + thisPK
							+ ", new value PK=" + newPK);
		}
		...
		if (modelEntity == null) {
			...
			if (modelEntity == null) {
				throw new IllegalStateException(
						"[GenericEntity.getModelEntity] could not find modelEntity for entityName " + entityName);
			}
		}
		return modelEntity;
	}
	...
	public Object get(String name) {
		if (getModelEntity().getField(name) == null) {
			throw new IllegalArgumentException("The field name (or key) [" + name + "] is not valid for entity ["
					+ this.getEntityName() + "].");
		}
	...
	public Object set(String name, Object value, boolean setIfNull) {
		...
		if (modelField == null) {
			throw new IllegalArgumentException("[GenericEntity.set] \"" + name + "\" is not a field of " + entityName
					+ ", must be one of: " + getModelEntity().fieldNameString());
		}
		if (value != null || setIfNull) {
			...
			if (value instanceof Boolean) {
				...
				if (!ObjectType.instanceOf(value, type.getJavaType())) {
					if (!("java.sql.Blob".equals(type.getJavaType()) && (value instanceof byte[] || ObjectType
							.instanceOf(value, ByteBuffer.class)))) {
						String errMsg =
								"In entity field [" + this.getEntityName() + "." + name
	...
	public byte[] getBytes(String name) {
		...
		if (value instanceof org.apache.ofbiz.entity.util.ByteWrapper) {
			...
			Debug.logWarning(
					"Found a ByteWrapper object in the database for field ["
							+ this.getEntityName()
							+ "."
							+ name
		...
		throw new IllegalArgumentException(
				"In call to getBytes the value is not a supported type, should be byte[] or ByteWrapper, is: "
	 ...
	 */
	private Object get(ModelEntity modelEntity, ModelEntity modelEntityToUse, String name, String resource,
			Locale locale) {
				...
				while (iterAlias != null && iterAlias.hasNext()) {
					...
					if (aliasField.getField().equals(curField.getName())) {
						ModelEntity memberModelEntity =
								modelViewEntity.getMemberModelEntity(aliasField.getEntityAlias());
	 ...
	 */
	public void setAllFields(Map<? extends Object, ? extends Object> fields, boolean setIfEmpty, String namePrefix,
			Boolean pks) {
		if (fields == null) {
			return;
		}
					...
					for (int i = 0; i < value.length(); i++) {
						...
						default:
							if (curChar < 0x20) {
								...
								Debug.logInfo("Removing invalid character [" + curChar + "] numeric value ["
										+ (int) curChar + "] for field " + name + " of entity with PK: "
										+ this.getPrimaryKey().toString(), module);
								...
								if (Debug.verboseOn()) {
									Debug.logVerbose("Entity: " + this.getEntityName() + ", PK: "
											+ this.getPrimaryKey().toString() + " -> char [" + curChar
											+ "] replaced with [" + replacement + "]", module);
								}
	...
	public Object getOriginalDbValue(String name) {
		if (getModelEntity().getField(name) == null) {
			throw new IllegalArgumentException("[GenericEntity.get] \"" + name + "\" is not a field of "
					+ getEntityName());
		}
					...
					if (insertDummy) {
						...
						for (ModelKeyMap mkm : relation.getKeyMaps()) {
							if (this.get(mkm.getFieldName()) != null) {
								...
								if (Debug.infoOn())
									Debug.logInfo(
											"Set [" + mkm.getRelFieldName() + "] to - "
													+ this.get(mkm.getFieldName()), module);
							} else {
								allFieldsSet = false;
							}
						}
