...
public class GenericDelegator implements Delegator {
	 ...
	 * think so yet); when true all operations without a transaction will be wrapped in one; seems to be necessary for
	 * some (all?) XA aware connection pools, and should improve overall stability and consistency
	 */
	...
	protected final AtomicReference<Future<DistributedCacheClear>> distributedCacheClear =
			new AtomicReference<Future<DistributedCacheClear>>();
	protected boolean warnNoEcaHandler = false;
	protected final AtomicReference<Future<EntityEcaHandler<?>>> entityEcaHandler =
			new AtomicReference<Future<EntityEcaHandler<?>>>();
	...
	/**
	 * A ThreadLocal variable to allow other methods to specify a user identifier (usually the userLoginId, though
	 * technically the Entity Engine doesn't know anything about the UserLogin entity)
	 */
	protected static ThreadLocal<List<String>> userIdentifierStack = new ThreadLocal<List<String>>();
	/**
	 * A ThreadLocal variable to allow other methods to specify a session identifier (usually the visitId, though
	 * technically the Entity Engine doesn't know anything about the Visit entity)
	 */
	...
	private static final AtomicReferenceFieldUpdater<GenericDelegator, LinkedBlockingDeque<?>> testOperationsUpdater =
			UtilGenerics.cast(AtomicReferenceFieldUpdater.newUpdater(GenericDelegator.class,
					LinkedBlockingDeque.class, "testOperations"));
	...
	protected GenericDelegator(String delegatorFullName) throws GenericEntityException {
		...
		if (UtilValidate.isNotEmpty(this.delegatorTenantId)) {
			...
			GenericValue tenant =
					EntityQuery.use(baseDelegator).from("Tenant").where("tenantId", this.delegatorTenantId)
							.cache(true).queryOne();
			if (tenant == null) {
				throw new GenericEntityException("No Tenant record found for delegator [" + this.delegatorFullName
						+ "] with tenantId [" + this.delegatorTenantId + "]");
			} else if ("Y".equals(tenant.getString("disabled"))) {
				throw new GenericEntityException("No Tenant record found for delegator [" + this.delegatorFullName
						+ "] with tenantId [" + this.delegatorTenantId + "]");
			}
			GenericValue kekValue =
					EntityQuery.use(baseDelegator).from("TenantKeyEncryptingKey")
							.where("tenantId", getDelegatorTenantId()).cache(true).queryOne();
			if (kekValue != null) {
		...
		if (warningList.size() > 0) {
			Debug.logWarning("=-=-=-=-= Found " + warningList.size()
					+ " warnings when checking the entity definitions:", module);
		...
		// NOTE: doing some things before the ECAs and such to make sure it is in place just in case it is used in a
		// service engine startup thing or something
	...
	private void initializeOneGenericHelper(String groupName) {
		...
		if (helperInfo == null) {
			if (Debug.infoOn()) {
				Debug.logInfo("Delegator \"" + delegatorFullName + "\" NOT initializing helper for entity group \""
						+ groupName + "\" because the group is not associated to this delegator.", module);
			}
			return;
		}
		...
		if (Debug.infoOn()) {
			Debug.logInfo("Delegator \"" + delegatorFullName + "\" initializing helper \"" + helperBaseName
					+ "\" for entity group \"" + groupName + "\".", module);
		}
			...
			try {
				...
				if (datasource.getCheckOnStart()) {
					...
					helper.checkDataSource(this.getModelEntityMapByGroup(groupName), null,
							datasource.getAddMissingOnStart());
				}
	...
	public synchronized void initEntityEcaHandler() {
		// Nothing to do if already assigned: the class loader has already been called, the class instantiated and
		// casted to EntityEcaHandler
	...
	protected EntityEcaHandler<?> createEntityEcaHandler() {
		// If useEntityEca is false do nothing: the entityEcaHandler member field with a null value would cause its code
		// to do nothing
		if (this.delegatorInfo.getEntityEcaEnabled()) {
			// time to do some tricks with manual class loading that resolves circular dependencies, like calling
			// services
			...
			try {
				...
				Debug.logWarning(e, "EntityEcaHandler class with name " + entityEcaHandlerClassName
						+ " could not be instantiated, Entity ECA Rules will be disabled", module);
			} catch (IllegalAccessException e) {
				Debug.logWarning(e, "EntityEcaHandler class with name " + entityEcaHandlerClassName
						+ " could not be accessed (illegal), Entity ECA Rules will be disabled", module);
	...
	public Map<String, ModelEntity> getModelEntityMapByGroup(String groupName) throws GenericEntityException {
		...
		if (this.delegatorInfo.getDefaultGroupName().equals(groupName)) {
			...
			for (String entityName : allEntityNames) {
				if (this.delegatorInfo.getDefaultGroupName().equals(
						getModelGroupReader().getEntityGroupName(entityName, this.delegatorBaseName))) {
	...
	public GenericHelperInfo getGroupHelperInfo(String entityGroupName) {
		...
		if (UtilValidate.isNotEmpty(this.delegatorTenantId) && "org.apache.ofbiz.tenant".equals(entityGroupName)) {
			Debug.logInfo("Can't access entity of entityGroup = " + entityGroupName + " using tenant delegator "
					+ this.getDelegatorName() + ", use base delegator instead", module);
			return null;
		}
		...
		if (UtilValidate.isNotEmpty(this.delegatorTenantId)) {
			...
			try {
				// NOTE: instead of caching the GenericHelpInfo object do a cached query here and create a new object
				// each time, will avoid issues when the database data changes during run time
				...
				GenericValue tenantDataSource =
						EntityQuery.use(baseDelegator).from("TenantDataSource")
								.where("tenantId", this.delegatorTenantId, "entityGroupName", entityGroupName)
								.cache(true).queryOne();
				...
				Debug.logInfo(e, "Error getting TenantDataSource info for tenantId=" + this.delegatorTenantId
						+ ", entityGroupName=" + entityGroupName, module);
			}
		}
		return helperInfo;
	}
	...
	public GenericHelper getEntityHelper(String entityName) throws GenericEntityException {
		...
		if (helperInfo != null) {
			...
			throw new GenericEntityException("There is no datasource (Helper) configured for the entity-group ["
					+ this.getEntityGroupName(entityName) + "]; was trying to find datasource (helper) for entity ["
					+ entityName + "]");
		}
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.apache.ofbiz.entity.Delegator#getEntityFieldType(org.apache.ofbiz.entity.model.ModelEntity,
	 * java.lang.String)
	 */
	@Override
	...
	public ModelFieldTypeReader getModelFieldTypeReader(ModelEntity entity) {
		...
		if (modelFieldTypeReader == null) {
			throw new IllegalArgumentException("ModelFieldTypeReader not found for entity " + entity.getEntityName()
	...
	public GenericValue makeValue(String entityName) {
		...
		if (entity == null) {
			throw new IllegalArgumentException("[GenericDelegator.makeValue] could not find entity for entityName: "
					+ entityName);
		}
	...
	public GenericValue makeValidValue(String entityName, Map<String, ? extends Object> fields) {
		...
		if (entity == null) {
			throw new IllegalArgumentException(
					"[GenericDelegator.makeValidValue] could not find entity for entityName: " + entityName);
		}
		GenericValue value = GenericValue.create(entity);
		value.setDelegator(this);
	...
	public GenericPK makePK(String entityName, Map<String, ? extends Object> fields) {
		ModelEntity entity = this.getModelEntity(entityName);
		if (entity == null) {
			throw new IllegalArgumentException("[GenericDelegator.makePK] could not find entity for entityName: "
					+ entityName);
		}
	...
	public GenericPK makePKSingle(String entityName, Object singlePkValue) {
		...
		if (entity == null) {
			throw new IllegalArgumentException(
					"[GenericDelegator.makePKSingle] could not find entity for entityName: " + entityName);
		}
	...
	public GenericValue createSetNextSeqId(GenericValue value) throws GenericEntityException {
		...
		value.setDelegator(this);
		// this will throw an IllegalArgumentException if the entity for the value does not have one pk field, or if it
		// already has a value set for the one pk field
		value.setNextSeqId();
		...
		try {
				...
				if (existingValue == null) {
					...
					if (Debug.infoOn()) {
						Debug.logInfo("Error creating entity record with a sequenced value [" + value.getPrimaryKey()
								+ "], trying again about to refresh bank for entity [" + value.getEntityName() + "]",
								module);
					}
					...
					if (Debug.infoOn()) {
						Debug.logInfo(
								"Successfully created new entity record on retry with a sequenced value ["
										+ value.getPrimaryKey() + "], after getting refreshed bank for entity ["
			...
			String errMsg =
					"Failure in createSetNextSeqId operation for entity [" + value.getEntityName() + "]: "
	...
	public GenericValue create(GenericValue value) throws GenericEntityException {
		...
		try {
			...
			String errMsg =
					"Failure in create operation for entity [" + (value != null ? value.getEntityName() : "null")
							+ "]: " + e.toString() + ". Rolling back transaction.";
	...
	public GenericValue createOrStore(GenericValue value) throws GenericEntityException {
		...
		try {
			...
			String errMsg =
					"Failure in createOrStore operation for entity [" + value.getEntityName() + "]: " + e.toString()
	...
	public int removeByPrimaryKey(GenericPK primaryKey) throws GenericEntityException {
		...
		try {
			...
			String errMsg =
					"Failure in removeByPrimaryKey operation for entity [" + primaryKey.getEntityName() + "]: "
							+ e.toString() + ". Rolling back transaction.";
			Debug.logError(e, errMsg, module);
			TransactionUtil.rollback(beganTransaction, errMsg, e);
			throw new GenericEntityException(e);
	...
	public int removeValue(GenericValue value) throws GenericEntityException {
		// NOTE: this does not call the GenericDelegator.removeByPrimaryKey method because it has more information to
		// pass to the ECA rule hander
		boolean beganTransaction = false;
		try {
			if (alwaysUseTransaction) {
				beganTransaction = TransactionUtil.begin();
			...
			// Need to call removedFromDatasource() here because the helper calls removedFromDatasource() on the PK
			// instead of the GenericEntity.
			...
			String errMsg =
					"Failure in removeValue operation for entity [" + value.getEntityName() + "]: " + e.toString()
							+ ". Rolling back transaction.";
			Debug.logError(e, errMsg, module);
			TransactionUtil.rollback(beganTransaction, errMsg, e);
			throw new GenericEntityException(e);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.apache.ofbiz.entity.Delegator#removeByCondition(java.lang.String,
	...
	public int removeByCondition(String entityName, EntityCondition condition) throws GenericEntityException {
		...
		try {
			...
			String errMsg =
					"Failure in removeByCondition operation for entity [" + entityName + "]: " + e.toString()
							+ ". Rolling back transaction.";
			Debug.logError(e, errMsg, module);
			TransactionUtil.rollback(beganTransaction, errMsg, e);
			throw new GenericEntityException(e);
	...
	public int removeRelated(String relationName, GenericValue value) throws GenericEntityException {
		...
		if (relation == null) {
			throw new GenericModelException("Could not find relation for relationName: " + relationName
					+ " for value " + value);
		}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.apache.ofbiz.entity.Delegator#storeByCondition(java.lang.String, java.util.Map,
	 * org.apache.ofbiz.entity.condition.EntityCondition)
	 */
	@Override
	public int storeByCondition(String entityName, Map<String, ? extends Object> fieldsToSet,
		...
		try {
			...
			String errMsg =
					"Failure in storeByCondition operation for entity [" + entityName + "]: " + e.toString()
							+ ". Rolling back transaction.";
			Debug.logError(e, errMsg, module);
			TransactionUtil.rollback(beganTransaction, errMsg, e);
			throw new GenericEntityException(e);
	...
	public int store(GenericValue value) throws GenericEntityException {
		...
		try {
			...
			String errMsg =
					"Failure in store operation for entity [" + value.getEntityName() + "]: " + e.toString()
							+ ". Rolling back transaction.";
			Debug.logError(e, errMsg, module);
			TransactionUtil.rollback(beganTransaction, errMsg, e);
			throw new GenericEntityException(e);
			...
			for (GenericValue value : values) {
				...
				if (!primaryKey.isPrimaryKey()) {
					throw new GenericModelException(
							"[GenericDelegator.storeAll] One of the passed primary keys is not a valid primary key: "
									+ primaryKey);
				}
	 ...
	 */
	@Override
	public GenericValue findOne(String entityName, Map<String, ? extends Object> fields, boolean useCache)
			throws GenericEntityException {
		GenericPK primaryKey = this.makePK(entityName, fields);
		if (!primaryKey.isPrimaryKey()) {
			throw new GenericModelException(
					"[GenericDelegator.findOne] Passed primary key is not a valid primary key: " + primaryKey);
		}
		...
		try {
			...
			ecaRunner.evalRules(EntityEcaHandler.EV_RETURN, EntityEcaHandler.OP_FIND, (value == null ? primaryKey
					: value), false);
			TransactionUtil.commit(beganTransaction);
			return value;
		} catch (Exception e) {
			String errMsg =
					"Failure in findOne operation for entity [" + entityName + "]: " + e.toString()
							+ ". Rolling back transaction.";
			Debug.logError(e, errMsg, module);
			TransactionUtil.rollback(beganTransaction, errMsg, e);
			throw new GenericEntityException(e);
	...
	public GenericValue findByPrimaryKeyPartial(GenericPK primaryKey, Set<String> keys) throws GenericEntityException {
		...
		try {
			...
			if (!primaryKey.isPrimaryKey()) {
				throw new GenericModelException(
						"[GenericDelegator.findByPrimaryKey] Passed primary key is not a valid primary key: "
								+ primaryKey);
			}
			...
			String errMsg =
					"Failure in findByPrimaryKeyPartial operation for entity [" + primaryKey.getEntityName() + "]: "
							+ e.toString() + ". Rolling back transaction.";
			Debug.logError(e, errMsg, module);
			TransactionUtil.rollback(beganTransaction, errMsg, e);
			throw new GenericEntityException(e);
	 ...
	 */
	@Override
	public List<GenericValue> findByAnd(String entityName, Map<String, ? extends Object> fields,
	 ...
	 * org.apache.ofbiz.entity.condition.EntityCondition, java.util.Set, java.util.List,
	...
	public EntityListIterator find(String entityName, EntityCondition whereEntityCondition,
		...
		// if there is no transaction throw an exception, we don't want to create a transaction here since closing it
		// would mess up the ELI
		if (!TransactionUtil.isTransactionInPlace()) {
			// throw new
			// GenericEntityException("ERROR: Cannot do a find that returns an EntityListIterator with no transaction in place. Wrap this call in a transaction.");

			// throwing an exception is a little harsh for now, just display a really big error message since we want to
			// get all of these fixed...
		...
		EntityListIterator eli =
				helper.findListIteratorByCondition(this, modelEntity, whereEntityCondition, havingEntityCondition,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.apache.ofbiz.entity.Delegator#findList(java.lang.String,
	 * org.apache.ofbiz.entity.condition.EntityCondition, java.util.Set, java.util.List,
	 * org.apache.ofbiz.entity.util.EntityFindOptions, boolean)
	 */
	@Override
	public List<GenericValue> findList(String entityName, EntityCondition entityCondition,
		...
		try {
			...
			try (EntityListIterator eli =
					this.find(entityName, entityCondition, null, fieldsToSelect, orderBy, findOptions)) {
			...
			String errMsg =
					"Failure in findByCondition operation for entity [" + entityName + "]: " + e.toString()
							+ ". Rolling back transaction.";
			Debug.logError(e, errMsg, module);
			TransactionUtil.rollback(beganTransaction, errMsg, e);
			throw new GenericEntityException(e);
	 ...
	 * 
	 * @see
	 * org.apache.ofbiz.entity.Delegator#findListIteratorByCondition(org.apache.ofbiz.entity.model.DynamicViewEntity,
	 * org.apache.ofbiz.entity.condition.EntityCondition, org.apache.ofbiz.entity.condition.EntityCondition,
	 * java.util.Collection, java.util.List, org.apache.ofbiz.entity.util.EntityFindOptions)
	 */
	@Override
	public EntityListIterator findListIteratorByCondition(DynamicViewEntity dynamicViewEntity,
		...
		EntityListIterator eli =
				helper.findListIteratorByCondition(this, modelViewEntity, whereEntityCondition,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.apache.ofbiz.entity.Delegator#findCountByCondition(java.lang.String,
	 * org.apache.ofbiz.entity.condition.EntityCondition, org.apache.ofbiz.entity.condition.EntityCondition,
	 * org.apache.ofbiz.entity.util.EntityFindOptions)
	 */
	@Override
	public long findCountByCondition(String entityName, EntityCondition whereEntityCondition,
		...
		try {
			...
			GenericHelper helper = getEntityHelper(modelEntity.getEntityName());
			long count =
					helper.findCountByCondition(this, modelEntity, whereEntityCondition, havingEntityCondition,
							findOptions);
			...
			String errMsg =
					"Failure in findListIteratorByCondition operation for entity [DynamicView]: " + e.toString()
							+ ". Rolling back transaction.";
			Debug.logError(e, errMsg, module);
			TransactionUtil.rollback(beganTransaction, errMsg, e);
			throw new GenericEntityException(e);
	 ...
	 */
	@Override
	public List<GenericValue> getMultiRelation(GenericValue value, String relationNameOne, String relationNameTwo,
		...
		try {
			...
			List<GenericValue> result =
					helper.findByMultiRelation(value, modelRelationOne, modelEntityOne, modelRelationTwo,
							modelEntityTwo, orderBy);
			TransactionUtil.commit(beganTransaction);
			return result;
		} catch (Exception e) {
			String errMsg =
					"Failure in getMultiRelation operation for entity [" + value.getEntityName() + "]: "
							+ e.toString() + ". Rolling back transaction.";
			Debug.logError(e, errMsg, module);
			TransactionUtil.rollback(beganTransaction, errMsg, e);
			throw new GenericEntityException(e);
	 ...
	 */
	@Override
	public List<GenericValue> getRelated(String relationName, Map<String, ? extends Object> byAndFields,
			List<String> orderBy, GenericValue value, boolean useCache) throws GenericEntityException {
		ModelEntity modelEntity = value.getModelEntity();
		ModelRelation relation = modelEntity.getRelation(relationName);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.apache.ofbiz.entity.Delegator#getRelatedDummyPK(java.lang.String, java.util.Map,
	 * org.apache.ofbiz.entity.GenericValue)
	 */
	@Override
	public GenericPK getRelatedDummyPK(String relationName, Map<String, ? extends Object> byAndFields,
			GenericValue value) throws GenericEntityException {
		ModelEntity modelEntity = value.getModelEntity();
		ModelRelation relation = modelEntity.getRelation(relationName);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.apache.ofbiz.entity.Delegator#getRelatedOne(java.lang.String, org.apache.ofbiz.entity.GenericValue,
	 * boolean)
	 */
	@Override
	public GenericValue getRelatedOne(String relationName, GenericValue value, boolean useCache)
			throws GenericEntityException {
		...
		if (!"one".equals(relation.getType()) && !"one-nofk".equals(relation.getType())) {
			throw new GenericModelException("Relation is not a 'one' or a 'one-nofk' relation: " + relationName
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.apache.ofbiz.entity.Delegator#clearCacheLineByCondition(java.lang.String,
	 * org.apache.ofbiz.entity.condition.EntityCondition)
	 */
	@Override
	 ...
	 * 
	 * @see org.apache.ofbiz.entity.Delegator#clearCacheLineByCondition(java.lang.String,
	 * org.apache.ofbiz.entity.condition.EntityCondition, boolean)
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.apache.ofbiz.entity.Delegator#putInPrimaryKeyCache(org.apache.ofbiz.entity.GenericPK,
	 * org.apache.ofbiz.entity.GenericValue)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * org.apache.ofbiz.entity.Delegator#setDistributedCacheClear(org.apache.ofbiz.entity.util.DistributedCacheClear)
	 */
	@Override
	 ...
	 */
	@Override
	public List<GenericValue> readXmlDocument(URL url) throws SAXException, ParserConfigurationException,
			java.io.IOException {
		if (url == null) {
			return null;
		}
	...
	protected static class EntityEcaRuleRunner<T> {
		...
		protected void evalRules(String event, String currentOperation, GenericEntity value, boolean isError)
				throws GenericEntityException {
			if (entityEcaHandler == null) {
				return;
			}
			// if (!"find".equals(currentOperation)) {
			// Debug.logWarning("evalRules for entity " + value.getEntityName() + ", currentOperation " +
			// currentOperation + ", event " + event, module);
			// }
	...
	protected static <T> EntityEcaRuleRunner<T> createEntityEcaRuleRunner(EntityEcaHandler<T> entityEcaHandler,
			String entityName) {
		return new EntityEcaRuleRunner<T>(entityEcaHandler,
				entityEcaHandler != null ? entityEcaHandler.getEntityEventMap(entityName) : null);
	}
	...
	public String getNextSeqId(String seqName, long staggerMax) {
		...
		if (nextSeqLong == null) {
			// NOTE: the getNextSeqIdLong method SHOULD throw a runtime exception when no sequence value is found, which
			// means we should never see it get here
	...
	public Long getNextSeqIdLong(String seqName, long staggerMax) {
		try {
			...
			if (sequencer == null) {
				...
				sequencer =
						new SequenceUtil(this.getEntityHelperInfo("SequenceValueItem"), seqEntity, "seqName", "seqId");
			...
			String errMsg =
					"Failure in getNextSeqIdLong operation for seqName [" + seqName + "]: " + e.toString()
							+ ". Rolling back transaction.";
			Debug.logError(e, errMsg, module);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.apache.ofbiz.entity.Delegator#setNextSubSeqId(org.apache.ofbiz.entity.GenericValue, java.lang.String,
	 * int, int)
	 */
	@Override
	public void setNextSubSeqId(GenericValue value, String seqFieldName, int numericPadding, int incrementBy) {
		if (value != null && UtilValidate.isEmpty(value.getString(seqFieldName))) {
			...
			try {
				...
				// Debug.logInfo("Get existing values from entity " + value.getEntityName() + " with lookupValue: " +
				// lookupValue + ", and the seqFieldName: " + seqFieldName + ", and the results are: " + allValues,
				// module);
				...
				for (GenericValue curValue : allValues) {
					...
					if (currentSeqId != null) {
						...
						try {
							...
							Debug.logWarning("Error in make-next-seq-id converting SeqId [" + currentSeqId
									+ "] in field: " + seqFieldName + " from entity: " + value.getEntityName()
									+ " to a number: " + e.toString(), module);
						}
					}
				}
				...
				String errMsg =
						"Failure in setNextSubSeqId operation for entity [" + value.getEntityName() + "]: "
								+ e.toString() + ". Rolling back transaction.";
				Debug.logError(e, errMsg, module);
				try {
	...
	@Override
	public Object encryptFieldValue(String entityName, ModelField.EncryptMethod encryptMethod, Object fieldValue)
	 ...
	 */
	@Override
	public Object decryptFieldValue(String entityName, ModelField.EncryptMethod encryptMethod, String encValue)
			throws EntityCryptoException {
		if (UtilValidate.isNotEmpty(encValue)) {
	...
	public Delegator cloneDelegator(String delegatorFullName) {
		...
		newDelegator.setDelegatorNames(UtilValidate.isNotEmpty(delegatorFullName) ? delegatorFullName
				: this.delegatorFullName);
	...
	private void storeForTestRollback(TestOperation testOperation) {
		if (!this.testMode || this.testRollbackInProgress) {
			throw new IllegalStateException(
					"An attempt was made to store a TestOperation during rollback or outside of test mode");
		}
	...
	public void initDistributedCacheClear() {
		// Nothing to do if already assigned: the class loader has already been called, the class instantiated and
		// casted to DistributedCacheClear
	...
	protected DistributedCacheClear createDistributedCacheClear() {
		// If useDistributedCacheClear is false do nothing: the distributedCacheClear member field with a null value
		// would cause dcc code to do nothing
		if (useDistributedCacheClear()) {
			...
			try {
				...
				Debug.logWarning(e, "DistributedCacheClear class with name " + distributedCacheClearClassName
						+ " could not be instantiated, distributed cache clearing will be disabled", module);
			} catch (IllegalAccessException e) {
				Debug.logWarning(e, "DistributedCacheClear class with name " + distributedCacheClearClassName
						+ " could not be accessed (illegal), distributed cache clearing will be disabled", module);
			} catch (ClassCastException e) {
				Debug.logWarning(e, "DistributedCacheClear class with name " + distributedCacheClearClassName
						+ " does not implement the DistributedCacheClear interface, "
						+ "distributed cache clearing will be disabled", module);
			}
		} else {
			Debug.logVerbose("Distributed Cache Clear System disabled for delegator [" + delegatorFullName + "]",
					module);
		}
		return null;
	}
