...
public final class TransactionUtil implements Status {
	...
	private static ThreadLocal<List<RollbackOnlyCause>> setRollbackOnlyCauseSave =
			new ThreadLocal<List<RollbackOnlyCause>>();
	...
	public static Map<Xid, DebugXaResource> debugResMap = Collections
			.<Xid, DebugXaResource> synchronizedMap(new HashMap<Xid, DebugXaResource>());
	...
	private static Map<Long, Exception> allThreadsTransactionBeginStack = Collections
			.<Long, Exception> synchronizedMap(new HashMap<Long, Exception>());
	private static Map<Long, List<Exception>> allThreadsTransactionBeginStackSave = Collections
			.<Long, List<Exception>> synchronizedMap(new HashMap<Long, List<Exception>>());

	private TransactionUtil() {
	}

	public static <V> V doNewTransaction(Callable<V> callable, String ifErrorMessage, int timeout,
	...
	public static <V> V
			doTransaction(Callable<V> callable, String ifErrorMessage, int timeout, boolean printException)
	...
	// }
	public static <V> InTransaction<V> inTransaction(Callable<V> callable, String ifErrorMessage, int timeout,
	...
	protected static void internalBegin(UserTransaction ut, int timeout) throws SystemException,
	...
	public static Connection enlistConnection(XAConnection xacon) throws GenericTransactionException {
		...
		try {
			...
			throw new GenericTransactionException(
					"SQL error, could not enlist connection in transaction even though transactions are available", e);
		}
	}
				...
				for (Exception stack : txBeginStackList) {
					Debug.logInfo(
							...
							"===================================================\n===================================================\n Tx begin stack history for thread ["
									+ curThreadId + "] history number [" + stackLevel + "]:", module);
					stackLevel++;
				}
			} else {
				Debug.logInfo(
						"========================================== No tx begin stack history found for thread ["
								+ curThreadId + "]", module);
			}
		}
	}
	...
	private static void setTransactionBeginStack(Exception newExc) {
		if (transactionBeginStack.get() != null) {
			...
			Debug.logWarning(
					e,
					"In setTransactionBeginStack a stack placeholder was already in place, here is where the transaction began: ",
					module);
			Exception e2 = new Exception("Current Stack Trace");
			Debug.logWarning(
					e2,
					"In setTransactionBeginStack a stack placeholder was already in place, here is the current location: ",
					module);
		}
	...
	private static Exception clearTransactionBeginStack() {
		...
		if (e == null) {
			...
			Debug.logWarning(
					e2,
					"In clearTransactionBeginStack no stack placeholder was in place, here is the current location: ",
					module);
			return null;
		} else {
	...
	public static Exception getTransactionBeginStack() {
		...
		if (e == null) {
			Exception e2 = new Exception("Current Stack Trace");
			Debug.logWarning(e2,
					"In getTransactionBeginStack no stack placeholder was in place, here is the current location: ",
					module);
		}
		return e;
	}
	...
	private static class RollbackOnlyCause {
		...
		public void logError(String message) {
			Debug.logError(this.getCauseThrowable(), (message == null ? "" : message) + this.getCauseMessage(),
					module);
		}
	...
	private static void setSetRollbackOnlyCause(RollbackOnlyCause newRoc) {
		if (setRollbackOnlyCause.get() != null) {
			...
			Debug.logWarning(
					e2,
					"In setSetRollbackOnlyCause a stack placeholder was already in place, here is the current location: ",
					module);
		}
	...
	public static RollbackOnlyCause getSetRollbackOnlyCause() {
		if (setRollbackOnlyCause.get() == null) {
			...
			Debug.logWarning(e,
					"In getSetRollbackOnlyCause no stack placeholder was in place, here is the current location: ",
					module);
		}
	 ...
	 */
	private static ThreadLocal<Map<Transaction, Timestamp>> suspendedTxStartStamps =
			new ThreadLocal<Map<Transaction, Timestamp>>() {
				@Override
	...
	*/
	...
	public static Timestamp getTransactionStartStamp() {
		...
		if (curStamp == null) {
			...
			try {
				...
				Debug.logError(
						e,
						"Error registering StampClearSync synchronization, stamps will still be reset if begin/commit/rollback are call through TransactionUtil, but not if otherwise",
						module);
			}
		}
		return curStamp;
	...
	public static Connection getCursorConnection(GenericHelperInfo helperInfo, Connection con) {
		...
		if (datasourceInfo == null) {
			Debug.logWarning("Could not find configuration for " + helperInfo.getHelperBaseName() + " datasource.",
					module);
			return con;
			...
			try {
				if (datasourceInfo.getResultFetchSize() > 1)
					con =
							CursorConnection.newCursorConnection(con, datasourceInfo.getProxyCursorName(),
									datasourceInfo.getResultFetchSize());
			} catch (Exception ex) {
				Debug.logWarning(ex, "Error creating the cursor connection proxy " + helperInfo.getHelperBaseName()
						+ " datasource.", module);
			}
		}
		return con;
